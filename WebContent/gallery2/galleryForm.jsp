...
<h3>이미지 파일 업로드 하기</h3>
<hr>
	<!--파일 업로드를 할대는 반드시 method="post" enctype="multipart/from-data"  -->
	<form action="gallerySave.jsp" method="post" enctype="multipart/form-data" onsubmit="return fileValid()" >
	<input type="text" name="title" placeholder="제목을 입력하세요."><br>
	<input type="file" name="pic" accept="image/jpeg,image/png" id="imgfile"><br>
	<!--웹에서는 파일의 형식을 확장자로만 구분하지 않고 MIME 형식을 사용합니다.  
	image/* ,image/*.png , video/* 등등
	accept 속성은 파일선택 창에서 초기형식. -> *.* 모든 파일로 변경 및 파일 잔송도 가능.
	-->
	<button>전송</button>
	</form>
	<!--  
	1. form 요소의 file 타입으로 클라이언트가 파일을 선택합니다.
	2. 서버 페이지에서는 file 타입요소로 전달된 파일을 다른 파라미터(입력값)과 함께 전송받습니다.(업로드)
		ㄴ 서버에서 업로드 폴더 설정이 필요 
	3. 2번에서 파일 전송에 필요한 Request 처리가 별도로 필여합니다. -> MultipartRequest 클래스			
	-->
