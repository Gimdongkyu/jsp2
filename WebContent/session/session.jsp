...
<div>http 프로토콜은 stateless(상태정보유지안됨) ,비연결지향  <br>
    (참고로 카카오톡은 연결지향.)
     -> 웹서버와 브라우저(사용자) 사이의 연결정보(사용자를 구분)를 저장하는 방법을 사용합니다. -> session 객체에 저장.<br>
...
<%
	...
	long create = session.getCreationTime();			//세션 객체가 만들어진 시간(첫번째 요청 시간)
	out.print("session 생성 시간 :" +create );				//타임스탬프(1970년 1월1일 기준 단위는 ms)
	out.print("<br>");
	...
	out.print("session 마지막 요청시간 :" + last );				
	out.print("<br>");
	out.print("session 마지막 요청시간 : "  + df.format(last));
	out.print("<br>");
